...
abstract class SpriteFit {
  ...
  /// The shader mode index used by [GridMesh]'s fragment-shader renderer.
  ///
  /// Maps to the per-cell `uCellModes` uniform in `grid_mesh.frag`:
  /// * 0 — stretch (non-uniform scale to fill)
  /// * 1 — tile (repeat pattern)
  /// * 2 — cover (uniform scale to fill, crop overflow)
  /// * 3 — contain (uniform scale to fit, letterbox)
  /// * 4 — fixed (original pixel size, aligned, transparent outside)
  int get meshMode => 0;

  /// The alignment used by cover, contain, and fixed shader modes.
  ///
  /// Coordinates are in the [-1, 1] × [-1, 1] range ([Alignment] convention).
  /// Ignored for stretch and tile modes. Defaults to [Alignment.center].
