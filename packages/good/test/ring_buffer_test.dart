...
void main() {
  group('RingBuffer', () {
    ...
    test('multiple writes drain in write order, in one batch', () {
      ...
      final records = ring.drain();
      expect(
        records.map((r) => r.recordType).toList(),
    ...
    test(
      'overflow: tryWrite returns false once the buffer is genuinely full',
      () {
        final ring = RingBuffer(
          32,
        ); // room for exactly two 8-byte (0-payload) records
        addTearDown(ring.dispose);
    ...
    test(
      'wraparound: writes that straddle the physical buffer end are handled',
      () {
    ...
    test('many small writes across many wraps preserve order and content', () {
      ...
      for (var batch = 0; batch < 50; batch++) {
        ...
        for (var i = 0; i < 5; i++) {
          ...
          expect(
            ring.tryWrite(batch * 10 + i, Uint8List.fromList(payload)),
            isTrue,
          );
