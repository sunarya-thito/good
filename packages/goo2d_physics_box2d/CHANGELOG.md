...
### Added
* **Six collision dispatchers on `Box2DPhysicsSystem`** -
  `collisionEnter2DEvent`, `collisionExit2DEvent`, `collisionStay2DEvent`,
  `triggerEnter2DEvent`, `triggerExit2DEvent`, `triggerStay2DEvent`, one per
  `CollisionListener` method (#384). A system may declare an event since #384,
  and this is where a contact is now fired from; the listeners were collected at
  boot, so `listenerCount` says whether there is anybody to tell before the work
  of a phase is done at all.

* **`goo2dPhysicsBox2dComponentBits`**, generated by `good_tool` into
  `lib/src/component_bits.g.dart` and exported from
  `goo2d_physics_box2d.dart` (#18). Name it to `Game.componentBits` and
  `Effector2D` and `RigidBody2D` hold the same query bits in every run and on
  every machine. It brings `goo2dComponentBits` with it, which brings
  `goodComponentBits`, so naming this one alone numbers all sixteen types the
  three packages register. See `package:good`'s entry for what a seeded
  registry does and does not pin.

* **A property per column, on the accessor.**
  `entity<RigidBody2D>().bodyGravityScale` reads that one entity's column, and
  assigning to it writes. `RigidBody2D` gains fourteen properties, generated by
  `good_tool` into
  `lib/src/accessors.g.dart` and exported from `goo2d_physics_box2d.dart`, so
  they arrive with the import you already have and there is nothing to run
  (#99, #300).

  The names follow the columns. `bodyType` stays `bodyType`: the property drops
  a prefix taken from the *component's* name, and this component is
  `RigidBody2D` while its columns are prefixed `body`, so nothing can tell that
  those go together. `Collider2D` and `Effector2D` get none - both declare
  their columns through their own hooks rather than as fields, so there is
  nothing for a parse to name.

  **For code touching one entity.** A system walking many resolves the
  component once per group and indexes the column, which is what it already
  did. Every read is of the published snapshot.
...
### Breaking
* **A contact is an event, and it reaches every `CollisionListener` in the
  game.** The system no longer resolves a listener off the entity: it used to
  cache one per shape at creation and call the six methods on it, so a contact
  reached only the two prefabs it involved (#384). It now fires the dispatcher
  for the phase, and whatever mixed `CollisionListener` into a `GameSystem`
  hears it. `CollisionListener` moved to `on GameListener` in `goo2d`, so a
  prefab that mixed it in stops compiling - see that package's entry for the
  conversion.

  Both sides of a contact still arrive, one delivery each, so a listener that
  tests `Collision2DEvent.sourceEntity` sees every contact its entities are in
  and never has to check `targetEntity` for the same entity. A listener counting
  *contacts* rather than *entities* sees each one twice.

  A game with no `CollisionListener` at all pays less than it did: both the
  per-pair delivery and the whole stay loop return on the dispatcher's listener
  count before either owner lookup, where the old path resolved both owners
  first and then found two null listeners. `dispatchStayEvents` is still there
  for the case where somebody is listening and the stay phase is not wanted.
