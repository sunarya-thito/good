...
/// arguments happen to select. That is what makes `goo compile --help` able to
...
class CommandRunner {
  CommandRunner(Command root, {StringSink? out})
    : _out = out ?? stdout,
      _root = _Node.declare(root, 'goo', null, out ?? stdout);
  ...
  Future<void> run(List<String> args) async {
    ...
    // asked for. Checked along the whole path, so `goo --help compile` works
    // as well as `goo compile --help`.
    ...
    // is what makes `goo --verbose compile windows` put `--verbose` on the
    ...
    // rather than throwing. `goo compile` asking `windows.selected` is an
  ...
  ///
  /// **Spec-aware, deliberately.** `goo compile --input-dir windows` must pass
