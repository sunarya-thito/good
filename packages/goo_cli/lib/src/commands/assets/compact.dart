...
import 'package:goo_cli/src/assets/compact.dart' as impl;
...
/// `goo assets compact` - source art in, canonical assets out.
///
...
class CompactCommand extends Command with Verbose {
  ...
  void _report(GooConfig config, Directory source, Directory output) {
    debug
      ..printf('source:  %s\n', [source.path])
      ..printf('output:  %s\n', [output.path])
  ...
  void _warnAboutUnlistedDirectories(
    ...
    info
      ...
      ..println(
        ...
        'and `goo generate` will not see them:',
      );
