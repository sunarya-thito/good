...
import 'package:goo_cli/src/assets/pack.dart';
import 'package:goo_cli/src/assets/strip.dart';
...
abstract class BuildSubCommand extends Command with Verbose {
  ...
  Future<void> execute() async {
    ...
    if (dryRun.value) {
      info
        ...
        ..println('  1. goo assets compact')
        ..println('  2. goo generate')
        ..printf('  3. goo assets pack --mode=%s\n', [assetMode.value.name])
  ...
  /// record of *which* files in the output directory goo generated - and
  ...
  Future<CompactPlan?> _compact(Directory project, GooConfig config) async {
  ...
  Future<bool> _pack(
    ...
    if (assetMode.value == AssetMode.release &&
      ...
      err.printf(
        ...
        'chunks would be built and never bundled. Add it - goo creates the '
        'directory itself.\n',
        [config.packOutput],
      );
      return false;
  ...
  void _stripLoose(Directory project, GooConfig config, CompactPlan compacted) {
    ...
    info.printf(
      ...
      '`goo assets compact` rebuilds them\n',
      [removed],
    );
  }
