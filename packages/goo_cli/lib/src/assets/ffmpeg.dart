...
enum FfmpegOrigin {
  /// `$GOO_FFMPEG` named it. First in the order so a project can pin a
...
/// # The order, and why
///
/// `$GOO_FFMPEG` -> `PATH` -> cache -> download. Every step before the last is
...
class FfmpegResolver {
  ...
  static const String overrideVariable = 'GOO_FFMPEG';

  Directory get cacheDir =>
      _cacheDir ?? Directory('${_home(_environment)}/.goo/ffmpeg');
  ...
  Future<Ffmpeg> resolve({
    ...
    if (pinned != null && pinned.isNotEmpty) {
      if (!_probe(pinned)) {
        ...
        throw FfmpegUnavailable(
          ...
          'Fix or unset it - goo will not silently fall back to a different '
    ...
    if (download == null) {
      throw FfmpegUnavailable(
        'ffmpeg is required to convert assets, and goo has no download for '
...
class FfmpegDownload {
  ...
  /// The download for the running platform, or null where goo has none.
  ///
  /// These are the long-standing community static-build hosts; goo does not
  ...
  /// than fetching quietly - and why `$GOO_FFMPEG` exists for anyone who would
