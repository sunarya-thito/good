/// Reads and rewrites `lib/goo.generated/asset_key.dart`.
///
/// The keys and the manifest share that one file because that is the shape
/// `goo generate` established, but they are produced by different commands on
...
/// because goo wrote the file itself, in a format it controls, and because the
...
List<int> readKeyMaterial(File assetKeyFile) {
  if (!assetKeyFile.existsSync()) {
    throw ArgumentError(
      'No ${assetKeyFile.path}. Run `goo generate` first - that is what '
  ...
  for (final name in const <String>[
    ...
    if (match == null) {
      throw ArgumentError(
        ...
        'hand, restore it - or delete it and re-run `goo generate`, which '
