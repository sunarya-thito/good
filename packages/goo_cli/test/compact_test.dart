...
Directory _tempDir() {
  final dir = Directory.systemTemp.createTempSync('goo_compact');
  addTearDown(() {
...
void main() {
  group('planCompaction', () {
    ...
    test('the source tree shape is preserved, never flattened', () {
      ...
      // `goo generate` derives collide - the source tree already said they
    ...
    test('a source directory with a trailing slash keeps whole filenames', () {
      // The bug this exists for: GooConfig normalises every directory to end
  ...
  group('FfmpegResolver ordering', () {
    ...
    test('GOO_FFMPEG wins over everything', () async {
      final resolved = await resolve(
        environment: const {'GOO_FFMPEG': '/pinned/ffmpeg'},
        probe: (_) => true,
      );
    ...
    test('a broken GOO_FFMPEG fails rather than falling back', () async {
      await expectLater(
        () => resolve(
          environment: const {'GOO_FFMPEG': '/pinned/ffmpeg'},
      ...
      await expectLater(
        ...
        throwsA(
          isA<FfmpegUnavailable>().having(
            (e) => e.message,
            'message',
            allOf(contains('--no-download'), contains('GOO_FFMPEG')),
          ),
        ),
      );
    });
        ...
        await expectLater(
          ...
          throwsA(
            isA<FfmpegUnavailable>().having(
              (e) => e.message,
              'message',
              allOf(contains('failed'), contains('GOO_FFMPEG')),
            ),
          ),
        );
      },
