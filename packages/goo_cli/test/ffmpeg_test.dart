...
Directory _tempDir() {
  final dir = Directory.systemTemp.createTempSync('goo_ffmpeg');
  addTearDown(() {
...
void main() {
  ...
  group('resolution order', () {
    test(r'$GOO_FFMPEG wins over everything else', () async {
      final resolved = await FfmpegResolver(
        environment: const <String, String>{'GOO_FFMPEG': '/pinned/ffmpeg'},
        cacheDir: _tempDir(),
    ...
    test(
      r'a broken $GOO_FFMPEG fails rather than falling back to PATH',
      () async {
        ...
        await expectLater(
          FfmpegResolver(
            environment: const <String, String>{'GOO_FFMPEG': '/gone/ffmpeg'},
            cacheDir: _tempDir(),
  ...
  group('the cache location', () {
    test('honours GOO_HOME, so a CI image can put it somewhere it keeps', () {
      final resolver = FfmpegResolver(
        environment: const <String, String>{'GOO_HOME': '/ci/cache'},
      );
      expect(resolver.cacheDir.path, '/ci/cache/.goo/ffmpeg');
    });
    ...
    test('falls back to the user profile', () {
      ...
      expect(resolver.cacheDir.path, '/home/dev/.goo/ffmpeg');
    });
  });

  test('every platform goo claims to support has a download', () {
    // `forHost` returning null is a legitimate answer for a platform goo has
