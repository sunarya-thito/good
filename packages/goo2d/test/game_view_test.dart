...
void main() {
  ...
  testWidgets('a tick drains, ingests and repaints - end to end', (
    ...
    expect(
      ...
      ['drawVertices'],
      reason:
          'paint() replays and does nothing else - no save/restore, no '
          'per-sprite call',
    );
  ...
  testWidgets('stopping while the view is still mounted disarms the renderer', (
    ...
    expect(
      ...
      greaterThan(0),
      reason:
          'the renderer samples on a self-rescheduling frame callback, '
    ...
    expect(
      ...
      0,
      reason:
          'Game.onStopped must have cancelled it - and it has to happen '
  ...
  testWidgets('a Game that has not started is refused with a reason', (
    ...
    expect(
      ...
      contains('unsendable'),
      reason:
          'the message has to say *why* - a tick listener registered '
