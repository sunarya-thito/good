...
Networking for the goo engine family: declared messages, sessions, and the
...
`ParamPointer`, `ParamBatch`, `ParamBuffer`) is `goo`'s own, reused rather
than reimplemented.
...
- `package:goo_net/testing.dart` — a conformance suite every backend is run
...
[`goo_net_p2p`](../goo_net_p2p) is the one that reaches another machine, with
