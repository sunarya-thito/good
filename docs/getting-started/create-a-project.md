...
Wrote ./my_game_bundle/lib/asset_key.dart
Resolving my_game_bundle - flutter pub get
0 texture(s), 0 audio file(s).
Generated 5 file(s) in my_game_bundle/.
my_game_bundle/ is good's, and lib/ is yours. The bindings import as
`package:my_game_bundle/textures.dart`.
...
Generation runs straight away instead of telling you to run it: the pubspec now
carries a path dependency on `my_game_bundle`, and a project whose bundle
package has never been written does not resolve at all — which makes "it does
not build" a new project's first experience.

`flutter pub get` is not in the next steps either. Generating runs it, because a
path dependency that was never resolved is the one mistake Flutter answers by
building green and shipping nothing.
...
├── my_game_bundle/                   ← generated; commit it, do not edit it
│   ├── .good_bundle                  ← says the directory is good's to rewrite
│   ├── pubspec.yaml                  ← every line generated
│   └── lib/
...
└── pubspec.yaml                      ← patched with the dependencies and asset entries
```

Everything good generates is in the package beside the project, and nothing good
generates is under `lib/`. Split by who wrote it rather than by what kind of
file it is, so "may this be overwritten" has one answer instead of two. The
`.good_bundle` marker is what makes the answer provable: without it, every good
...
!!! info "`my_game_bundle/` should be committed"
...
Both asset directories must be listed — that list is the only thing Flutter
bundles from.

`good: bundle:` is the answer to "which package is the bundle", and it is
written rather than worked out each run. Rename the project to `cool_game` and
that line still names `my_game_bundle`, which is the directory that exists, the
one `dependencies:` points at, and the one your imports spell. Recomputing the
name would build `cool_game_bundle` beside it: two packages, both resolving,
one of them dead, and nothing in the build saying which answered.

If the pubspec is not a shape the patcher recognises, it declines to edit and
prints the lines to add by hand instead; editing someone's pubspec blind is not
