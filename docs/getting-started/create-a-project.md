...
`goo create` scaffolds a Flutter app wired up to goo: a `Game`, a `GameState`,
...
Wrote ./lib/goo.generated/asset_key.dart
0 texture(s), 0 audio file(s).
Generated 4 file(s) in lib/goo.generated/.
...
project's `lib/goo.generated/` would otherwise be missing, so `main.dart` would
...
| `--no-flutter-create` | off | Write only the goo files, into a Flutter project that already exists |
...
rewrites platform folders. Use `--no-flutter-create` to add the goo files to a
...
Three of the four generated files are rewritten on every `goo generate`.
...
already built. See [`--rotate-keys`](../reference/cli.md#goo-generate).

!!! info "`goo.generated/` should be committed"
...
`goo create` edits the pubspec textually rather than through a YAML round-trip,
...
## Adding goo to an existing project

`--no-flutter-create` writes the goo files into a Flutter project that already
...
goo reads its settings from a `goo:` section of the pubspec — not a second
`goo.yaml` beside it. A project already has one file that says what it is and
...
no `goo:` section at all works — a new project should run before anyone has
